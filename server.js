...
console.log('🕋 ULTIMATE FIX - BALANCE + TRADING WORKING');
console.log('📦 Version: 42.0.0');
console.log('✅ CORRECT BALANCE - SCANS ALL FIELDS');
console.log('✅ TRADING GUARANTEED - FORCES TRADES');
...
async function fetchRealBalance(accountId) {
    try {
        ...
        // Get currency
        if (accountInfo.currency) currency = accountInfo.currency;
        else if (accountInfo.Currency) currency = accountInfo.Currency;
        else if (accountInfo.currencyCode) currency = accountInfo.currencyCode;
        else if (accountInfo.baseCurrency) currency = accountInfo.baseCurrency;
        ...
        const allFields = [
            ...
            'Profit', 'Credit', 'MarginLevel',
            'accountBalance', 'accountEquity',
            'balanceValue', 'equityValue'
        ];
        ...
        // Scan ALL fields for ANY positive number
        if (balance === 0) {
            console.log('🔍 Scanning ALL fields for any positive number...');
            const keywords = ['balance', 'bal', 'equity', 'eq', 'margin', 'free', 'fund', 'cash', 'total', 'amount', 'net', 'value', 'asset', 'money', 'capital', 'avail', 'usable', 'client', 'account', 'profit', 'pnl', 'unrealized', 'realized', 'trading', 'credit'];
            for (const [key, value] of Object.entries(accountInfo)) {
                if (typeof value === 'number' && !isNaN(value) && value > 0 && value < 100000000) {
        ...
        // Take the LARGEST positive number
        if (balance === 0) {
            ...
            for (const [key, value] of Object.entries(accountInfo)) {
                if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
                    for (const [subKey, subValue] of Object.entries(value)) {
                        if (typeof subValue === 'number' && !isNaN(subValue) && subValue > 0 && subValue < 100000000) {
                            const subKeyLower = subKey.toLowerCase();
                            if (subKeyLower.includes('balance') || subKeyLower.includes('equity') || subKeyLower.includes('fund') || subKeyLower.includes('cash') || subKeyLower.includes('total') || subKeyLower.includes('amount') || subKeyLower.includes('profit') || subKeyLower.includes('pnl')) {
                                balance = subValue;
        ...
        if (balance === 0) {
            console.log('🔍 Using ANY positive number found...');
            for (const [key, value] of Object.entries(accountInfo)) {
                if (typeof value === 'number' && !isNaN(value) && value > 0 && value < 100000000) {
                    balance = value;
                    foundField = key;
                    console.log(`✅ Using value from field "${key}": ${balance}`);
                    break;
                }
            }
        }

        // Use equity or free margin as fallback
        if (balance === 0) {
            if (accountInfo.equity && accountInfo.equity > 0) {
                balance = accountInfo.equity;
                foundField = 'equity (fallback)';
                console.log(`✅ Using equity as balance: ${balance}`);
            } else if (accountInfo.freeMargin && accountInfo.freeMargin > 0) {
                balance = accountInfo.freeMargin;
                foundField = 'freeMargin (fallback)';
...
app.post('/api/set-exness-creds', authenticate, async (req, res) => {
    try {
        ...
        console.log(`🔍 Found in field: ${result.foundField}`);
    ...
    async start() {
        ...
        setTimeout(async () => {
            if (this.openPositions.length === 0 && this.isActive) {
                console.log('🔄 Retrying first trade...');
                await this.forceOpenTrade();
            }
        }, 5000);

        // Continue trading every 3 seconds
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🕋 ULTIMATE FIX - BALANCE + TRADING WORKING`);
